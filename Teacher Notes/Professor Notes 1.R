...
#assign a training set
train = data.frame(lstat,medv)
#assign a testing set
test = data.frame(lstat,medv)
ind = order(test[,1])
test =test[ind,]


#defines MSE variable for all code
MSE = NULL

#choosing different k values
kk = c(2,10,50,100,150,200,250,300,400,505)

#for loop for the differnt k values
for(i in kk){
  
  #funtion of the knn algorithm
  #y variable given first ~ per x what we're dividing by,training set, testing set,iteration, kernel=neighbors weight
...
#only to visualize a concept - would not necessarily include in code
for(i in seq(1,505,by=100)){
  #gives is the closest 20 neighbors
  ii = near$C[i,1:20]
  plot(lstat,medv,main=paste("k=",20),pch=19,cex=0.8,col="darkgray")
  lines(test[,1],near$fitted,col=2,lwd=2)
  #abline is used to create additional lines
  #abline:v = vertical, lty = line type
...
#pulls random sample from the data set
tr = sample(1:506,400)

#to set up the training set and the testing set
train = train[tr,]
test = test[-tr,]
...
#find the minimum mean squared error of all test cases
best = which.min(out_MSE)

#Plot of the RMSE out-of-sample
...
#apply takes a function and applies it to all row or column into a matrix
...
for(j in 1:kcv){
  ...
  #taking off chunks of the set length
...
#Must load original libraries from top of code

data(fgl) ## loads the data into R; see help(fgl)
attach(fgl)

#to do multiple plots on the same graph
#mfcol will go by columns where mfrow goes by row
par(mfrow=c(2,3))

#all of the following give box plots because type is a text name, not numberic value
...
ca <- read.csv("/Users/brettscroggins/Downloads/CAhousing.csv")
